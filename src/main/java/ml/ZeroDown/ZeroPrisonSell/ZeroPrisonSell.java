...
public class ZeroPrisonSell extends JavaPlugin {
    ...
    public static void sellItems(String b, Player player) {
        ...
        player.sendMessage(ChatColor.GRAY + " > " + languageconfig.getString("sold-success").replace("%e", String.valueOf(itemint)));
        itemint = 0;
        player.sendMessage("");
        player.sendMessage(ChatColor.GRAY + " > " + languageconfig.getString("price-multiplied").replace("%e", String.valueOf(playerconfig.getInt("multiplier"))) + "x");
        player.sendMessage("");
        player.sendMessage(ChatColor.GRAY + " > " + languageconfig.getString("price-recieved").replace("%e", finalprice));
        player.sendMessage(ChatColor.GRAY + "" + ChatColor.STRIKETHROUGH + "---------------" + ChatColor.RESET + " " + ChatColor.translateAlternateColorCodes('&', languageconfig.getString("prefix")) + " " + ChatColor.GRAY + ChatColor.STRIKETHROUGH + "---------------");
    }
